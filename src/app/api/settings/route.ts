...
import { gmailConfigurationStatus } from "@/lib/gmail-oauth";
import { structuredProfileSchema } from "@/lib/profile-schema";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const gmailConfig = gmailConfigurationStatus(request.nextUrl.origin);
    return ok({ profile, integrations: { gmail: { connected: Boolean(gmail), email: gmail?.accountEmail, ...gmailConfig }, tavily: !!tavily || !!process.env.TAVILY_API_KEY, gemini: !!gemini || !!process.env.GEMINI_API_KEY } });
